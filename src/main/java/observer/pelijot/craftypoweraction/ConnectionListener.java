...
import observer.pelijot.craftypoweraction.component.RunCommand;
...
public class ConnectionListener {
    ...
    private boolean isReachable(RegisteredServer server) {
        try {
            ...
            logger.error("Server '{}' does not have its Crafty ID configured in the plugin's configuration", server.getServerInfo().getName(), exception);
            return false;
        } catch (IllegalArgumentException exception) {
            logger.error("The Crafty URL is missing or invalid in the plugin's configuration", exception);
            return false;
        }
    }
}
